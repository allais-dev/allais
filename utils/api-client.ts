...
export class ApiClient {
      ...
      while (attempt < MAX_RETRIES && !responseReceived && this.lastRequestId === requestId) {
        ...
        try {
          ...
          if (
            ...
            finalText.includes("I'm thinking about this...") ||
            finalText.includes("Working on your question...") ||
            finalText.includes("Processing your request...") ||
            finalText.includes("Let me analyze that...")
          ) {
            ...
            if (/[\d\s+\-*/()]+/.test(message) && /[+\-*/]/.test(message)) {
              try {
                ...
                finalText = `🧮 Calculation Result\n\n\n${mathExpression} = ${result}`
              } catch (error) {
                console.error(`[${requestId}] Failed to calculate math expression:`, error)
                finalText = "I'm having trouble calculating that right now. Please try again."
              }
            } else {
  ...
  private generateFallbackResponse(message: string, conversationHistory: Message[]): string {
    ...
    if (message.toLowerCase().includes("and") && /^\d+$/.test(message.replace(/and/i, "").trim())) {
      ...
      if (!isNaN(number)) {
        return `The next number after ${number} is ${number + 1}.`
      }
    }
    ...
    if (/what can you do|your capabilities|help me with/i.test(message)) {
      return "I can help with answering questions, writing content, explaining concepts, and much more. What would you like assistance with?"
    }

    // Default response
    return "I understand your question. Let me provide a helpful response based on what I know. If this doesn't fully address your question, please feel free to ask for more details."
  }
  ...
  private async startTypingSimulation(
    ...
    // Generate a plausible initial response based on the prompt
        ...
        try {
          ...
          try {
            ...
            if (
              ...
              text.includes("I'm thinking about this...") ||
              text.includes("Working on your question...") ||
              text.includes("Processing your request...") ||
              text.includes("Let me analyze that...")
            ) {
              // For math expressions, provide a direct answer
              if (/[\d\s+\-*/()]+/.test(message) && /[+\-*/]/.test(message)) {
                try {
                  ...
                  console.error(`[${requestId}] Failed to calculate math expression:`, error)
                  text = "I'm having trouble calculating that right now. Please try again."
                }
              } else {
                // Generate a fallback response
